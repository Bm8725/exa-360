...
// ── Diagramă mimică (P&ID) a liniei de spălare ── semnătura vizuală clasică HMI Siemens
function Mimic() {
  ...
  return (
    <svg viewBox="0 0 1000 220" style={{ width: "100%", height: "auto", display: "block" }}>
      ...
      {STATII.map((s, i) => {
        ...
        return (
          <g key={s.key}>
            {/* conector vertical spre conductă */}
            <line x1={cx} y1={y + 34} x2={cx} y2={pipeY} stroke="#8A929B" strokeWidth="3" />

            {/* iconiță proces în funcție de tip */}
            {s.tip === "tank" && (
              <g>
                <rect x={cx - 22} y={y - 20} width="44" height="54" fill="#F4F5F6" stroke={col} strokeWidth="2.5" />
                <rect x={cx - 22} y={y + s.stare === "idle" ? 14 : 4} width="44" height={s.stare === "idle" ? 20 : 30} fill={col} opacity="0.35" />
              </g>
            )}
            {s.tip === "motor" && (
              <g>
            ...
            {/* vehicul — poziția curentă pe linie */}
            {s.activ && (
              <g>
...
function IOField({ label, val, unit }) {
  return (
...
export default function IndustrialDashboard() {
  ...
  const [tab, setTab] = useState("PROCES");
  ...
  return (
    <div style={{ background: C.bg, minHeight: "100vh", fontFamily: "Arial, 'Segoe UI', sans-serif", color: C.text }}>
      ...
      <div style={{ display: "flex", gap: 10, padding: 12 }}>
        ...
        {/* taste funcționale F1–F8, stil panou Comfort */}
